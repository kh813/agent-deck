...
**修正（2026-08-14）:** ZIP に `Initial_setup_for_Mac.command`（`agent-deck.app` と同階層）を同梱し、初回だけこれをダブルクリックして起動する運用に変更しました（`.github/workflows/release.yml` の macOS zip ステップ参照）。中身は `xattr -cr ./agent-deck.app && open ./agent-deck.app` のみ — 起動前に quarantine を解除するため、App Translocation が発生せず、作業ディレクトリも安定します。一度実行すれば `agent-deck.app` 自体の quarantine が解除済みになるため、2回目以降は `agent-deck.app` を直接ダブルクリックするだけで良く、実行後は自身を `tmp/` フォルダへ移動してメインフォルダに残り続けないようにしています。Windows 版ZIP には最初から同梱していませんが、`package_release.py` が mac/win 両ZIPをマージする社内配布ZIPでは（§7c）Windows 環境にもこのファイルが紛れ込むため、`preflight.bat` が起動のたびに削除するようにしています。
...
**Fix (2026-08-14):** the ZIP now bundles `Initial_setup_for_Mac.command` (sibling to `agent-deck.app`) to double-click once on first launch — see the macOS zip step in `.github/workflows/release.yml`. It's just `xattr -cr ./agent-deck.app && open ./agent-deck.app`, clearing quarantine before the app ever runs, so App Translocation never triggers and the working directory stays stable. Once run, `agent-deck.app` itself is no longer quarantined, so subsequent launches can just double-click it directly — the script moves itself into `tmp/` afterward so it doesn't linger in the main folder. It's never added to the Windows-only GitHub zip, but `package_release.py`'s org-Drive distribution (§7c) merges both platform zips together, so it can still end up in a Windows install that way -- `preflight.bat` deletes it on every launch to clean that up.
